...
def merge_sorted_lists(list_a: LinkedList, list_b: LinkedList) -> LinkedList:
    ...
    while curr_a != None and curr_b != None:
        while curr_a.data >= curr_b.data and curr_b:
            print(f'Current A: {curr_a.data} Current B: {curr_b.data}')
            curr_new_list.next = curr_b
            curr_b = curr_b.next
            curr_new_list = curr_new_list.next
            print(f'New List: {curr_new_list.data}')
        
        while curr_a.data <= curr_b.data and curr_a:
            print(f'Current A: {curr_a.data} Current B: {curr_b.data}')
